...
import anonymization.KMatchAnonymizer;
	...
	public static void oneRunExperimentRobustSybilsRandomNetworks(int n, int m, int attackType, int attackersCount, int k, int maxEditDist, String fileNameOutOriginalWalkBased,
		...
		if (attackSimulator != null) {
			...
			for (int i = 1; i <= total ; i++) {
				...
				KMatchAnonymizer.anonymizeGraph(anonKAutomorphicGraphWalkBased, k);
				...
				KMatchAnonymizer.anonymizeGraph(anonKTimesHalfEllAutomorphicGraphWalkBased, k * (attackersCount / 2));
				...
				KMatchAnonymizer.anonymizeGraph(anonKTimesEllAutomorphicGraphWalkBased, k * attackersCount);
...
//				outsAdjTransformed[k-2] = new FileWriter(fileNamesOutAdjTransformed[k-2] + ".dat", true);
		...
		for (int i = 0; i < 1000; i++) {	 // Although it is the same graph, every iteration shuffles its vertex set, so the attacker targets different victims 
			...
			KMatchAnonymizer.anonymizeGraph(graphKAutomorphic, k);
			...
			KMatchAnonymizer.anonymizeGraph(graphKTimesHalfEllAutomorphic, k * (attackerCount / 2));
			...
			KMatchAnonymizer.anonymizeGraph(graphKTimesEllAutomorphic, k * attackerCount);
